...
def main():
    ...
    if uploaded_file:
                ...
                if result:
                    ...
                    for i, doc in enumerate(result['source_documents']):
                        with st.expander(f"Source {i+1} from {doc.metadata['document_title']}"):
                            ...
                            hyperlink = f"[View Paper]({doc.metadata['document_id']})"
        ...
        # Documents Tab
        with tab2:
            st.subheader("Primary and Referenced Documents")
            primary_doc = st.session_state.doc_manager.primary_doc
            referenced_docs = st.session_state.doc_manager.referenced_docs

            st.write("### Primary Document")
            st.write(primary_doc['title'])

            st.write("### Referenced Documents")
            for ref_path, doc_info in referenced_docs.items():
                st.write(f"- {doc_info['title']} ({ref_path})")
